...
export default function MissionBoard({
  ...
  const boardItems: BoardItem[] = missions.map((mission) => {
    return {
      ...
      badges: [
        // 주차 배지만 표시
        <WeekBadge key='week' week={mission.week} size='md' variant='gradient' theme='slate' format='W' />,
      ].filter(Boolean),
    };
  });
            ...
            <div key={item.id} className='p-6 hover:bg-slate-50 transition-colors'>
              <div className='flex items-start justify-between'>
                <div className='flex-1 min-w-0'>
                  ...
                  {item.content && <p className='text-sm text-slate-600 mb-2 line-clamp-2'>{item.content}</p>}
