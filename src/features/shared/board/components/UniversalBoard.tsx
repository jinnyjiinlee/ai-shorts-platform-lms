...
interface UniversalBoardProps {
  ...
  viewType?: 'board' | 'card';
  onViewTypeChange?: (type: 'board' | 'card') => void;
  cardComponent?: React.ComponentType<{
    items: any[];
    onItemClick: (item: any) => void;
  }>;
  cardData?: any[]; // 카드뷰용 원본 데이터
}
...
export default function UniversalBoard({
  ...
  return (
    <div className='space-y-6'>
          ...
          {showViewToggle && onViewTypeChange && (
            <div className='flex items-center space-x-1 bg-slate-100 rounded-lg p-1'>
              <button
                ...
                className={`flex items-center space-x-1 px-3 py-2 rounded-md text-sm transition-all ${
                  viewType === 'card' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'
                }`}
              >
              ...
              <button
                ...
                className={`flex items-center space-x-1 px-3 py-2 rounded-md text-sm transition-all ${
                  viewType === 'board' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'
                }`}
              >
      ...
      {/* 아이템 목록 */}
      {showViewToggle && viewType === 'card' && CardComponent && cardData ? (
        <CardComponent
          items={cardData}
          onItemClick={
            onViewItem
              ? (item: any) => {
                  // cardData의 원본 객체를 BoardItem으로 변환해서 onViewItem 호출
                  const boardItem = items.find((boardItem) => boardItem.id === item.id);
                  if (boardItem && onViewItem) {
                    onViewItem(boardItem);
                  }
                }
              : () => {}
          }
          ...
          {items.length === 0 ? (
            <div className='p-12 text-center text-slate-500'>
              ...
              <p className='text-lg mb-2'>
                {userRole === 'admin' ? '아직 작성된 글이 없습니다.' : '현재 글이 없습니다.'}
              </p>
              {userRole === 'admin' && onCreateItem && (
                <Button onClick={onCreateItem} variant='primary' className='mt-4'>
                  첫 번째 글 작성하기
                </Button>
              )}
            </div>
