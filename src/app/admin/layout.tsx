...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className='flex h-screen bg-gradient-to-br from-slate-50 to-blue-50/30'>
      ...
      {/* 사이드바 */}
      <div
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-white/90 backdrop-blur-md border-r border-slate-200/50 shadow-xl transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0 ${
      ...
      <div className='flex-1 flex flex-col overflow-hidden lg:ml-0'>
        ...
        {/* 콘텐츠 영역 */}
        <main className='flex-1 overflow-x-hidden overflow-y-auto bg-slate-50 p-4 lg:p-6'>{children}</main>
      </div>
    </div>
  );
}
