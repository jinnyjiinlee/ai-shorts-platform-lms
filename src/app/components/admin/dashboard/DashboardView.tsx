...
import { fetchDashboardStats, fetchCohortData, DashboardStats, CohortDashboardData } from '../../../../lib/services/dashboard/adminDashboardService';
...
const convertToCohortData = (dashboardData: CohortDashboardData): CohortData => {
  return {
    ...
    statusColor: dashboardData.status === 'active' ? 'bg-green-100 text-green-800' : 
                 dashboardData.status === 'completed' ? 'bg-gray-100 text-gray-800' : 
...
export default function DashboardView() {
  ...
  const overallStats: OverallStats = {
    ...
    averageSubmissionRate: displayCohortData.length ? Math.round(displayCohortData.reduce((s, c) => s + c.submissionRate, 0) / displayCohortData.length) : 0,
