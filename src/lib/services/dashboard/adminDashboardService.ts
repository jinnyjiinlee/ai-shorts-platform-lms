...
export const fetchDashboardStats = async (): Promise<DashboardStats> => {
  try {
    ...
    const averageSubmissionRate = totalExpectedSubmissions > 0 
  ...
  try {
    ...
    (missions || []).forEach(mission => {
      ...
      if (existingWeek) {
        ...
        existingWeek.rate = existingWeek.total > 0 
          ? Math.round((existingWeek.submitted / existingWeek.total) * 100) 
          : 0;
      } else {
        cohortData.weeklySubmissions.push({
          ...
          rate: cohortData.totalStudents > 0 
            ? Math.round((submittedCount / cohortData.totalStudents) * 100) 
            : 0
        });
      }
    });
    ...
    cohortMap.forEach((cohortData, cohort) => {
      ...
      cohortData.submissionRate = totalExpected > 0 
